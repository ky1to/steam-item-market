...
from schemas import SItemAdd, SItemPrice
...
def get_links(page_number: int, id_game: int):
    ...
    headers = {
        ...
        # 'Cookie': 'timezoneOffset=10800,0; browserid=2694894590571411386; recentlyVisitedAppHubs=730^%^2C1407200^%^2C431960^%^2C394360^%^2C252490; wants_mature_content_apps=252490; strInventoryLastContext=252490_2; sessionid=9456c9a806560170bd6a17f2; steamCountry=BY^%^7C62028401c15f5006ba9a1f2ceb8fb5d5',
...
def get_price_data_item(path: str, item_nameid: str, id_game: int):
    try:
        headers = {
            ...
            'Sec-Fetch-Site': 'same-origin',
            'If-Modified-Since': '',
            'Referer': 'https://steamcommunity.com/market/search?appid={id_game}',
            'Sec-Fetch-Dest': 'empty',
            'Sec-Fetch-Mode': 'cors',
            'Sec-Fetch-Site': 'same-origin',
        }
        ...
        print(colored(f'Error 2 get price  steam {response.status_code}', 'red'))
        time.sleep(10)
        return get_price_data_item(path, item_nameid, id_game)
...
def fix_data(data, item_db, game_id):
    try:
        if data == None:
            time.sleep(1)
            data = get_price_data_item(item_db.link, item_db.item_nameid, game_id)
            data = fix_data(data, item_db, game_id)
        a = search_lots(data["sell_order_summary"])
        a = search_lots(data["buy_order_summary"])
        return data
    except:
        time.sleep(1)
        data = get_price_data_item(item_db.link, item_db.item_nameid, game_id)
...
async def update_price():
    game_id = 252490
    while True:
        data_db = await Interface.find_all()
        n = 0
        for item_db in data_db:
            ...
            print(f"{item_db.name} {round(n/len(data_db)*100, 3)}%")

if __name__ == "__main__":
    loop = asyncio.get_event_loop()
    inp = input("1 - update db; 2 - update price ")
    if inp == "1":
